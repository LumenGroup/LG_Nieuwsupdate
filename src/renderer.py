...
def _logo_wit_data_uri() -> str:
    pad = Path(__file__).resolve().parent.parent / "media" / "tekst_groot_wit.png"
    data = base64.b64encode(pad.read_bytes()).decode("ascii")
    return f"data:image/png;base64,{data}"

def _logo_paars_data_uri() -> str:
    pad = Path(__file__).resolve().parent.parent / "media" / "tekst_groot.png"
    data = base64.b64encode(pad.read_bytes()).decode("ascii")
    return f"data:image/png;base64,{data}"

def _favicon_data_uri() -> str:
    pad = Path(__file__).resolve().parent.parent / "media" / "favicon.png"
    data = base64.b64encode(pad.read_bytes()).decode("ascii")
    return f"data:image/png;base64,{data}"
...
<body>
  <header class="hero">
    <div class="hero-inner">
      <span class="merk">Privacy & nformatiebeveiliging nieuwsupdate</span>
      <a href="https://www.lumengroup.nl" target="_blank" rel="noopener">
        <img src="{logo_wit}" alt="{_esc(config.NIEUWSBRIEF_TITEL)}" class="logo_groot">
      </a>
  ...
  <main class="wrap">
    ...
    <footer>
      <p>Deze nieuwsupdate is automatisch samengesteld uit RSS-feeds van openbare bronnen, als de <strong>Autoriteit Persoonsgegevens</strong>, de <strong>European Data Protection Board</strong>, <strong> Nationaal Cyber Security Centrum</strong>, <strong>Privacynieuws.nl</strong>, <strong>Security.nl</strong>, <strong>Rechtspraak.nl</strong> en <strong>DPcuria.eu</strong> (HvJ-EU).</p>
      <p>Samenstelling gebeurt geautomatiseerd. Controleer altijd de oorspronkelijke bron.</p>
      <a href="https://www.lumengroup.nl" target="_blank" rel="noopener">
        <img src="{logo_paars}" class="logo_klein">
      </a>
    </footer>
  </main>
</body>
